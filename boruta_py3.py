...
class BorutaPy3(object):
    ...
    def __init__(self, estimator, n_estimators=1000, perc=99,
                 alpha=8, beta=2, bf_thresh=30, max_iter=100, verbose=0):
    ...
    def _fit(self, X, y):
        ...
        self.priors = {}
        self.theta = np.linspace(0, 1, 500)        
        # setup bayesian priors        
        for f in range(n_feat):
            self.priors[f] = {'alpha': self.alpha, 'beta': self.beta}
            self.prior = sp.stats.beta.pdf(self.theta, self.alpha, self.beta)
        ...
        while np.any(dec_reg == 0) and iter < self.max_iter:
            ...
            if self.n_estimators == 'auto':
                not_rejected = active_features.shape[0]
            ...
            # cur_sha_diff = cur_imp[0][active_features] - cur_imp[1]
            # cur_sha = cur_imp[1][np.where(cur_sha_diff >= 0)[0]]
        ...
        confirmed = np.where(dec_reg > 0)[0]
    ...
    def _do_tests(self, dec_reg, hit_reg, iter):
        bayes_features = np.where(dec_reg >= 0)[0]
        for i, f in enumerate(bayes_features):
            h = hit_reg[bayes_features][i]
            posterior = sp.stats.beta.pdf(self.theta, self.priors[f]['alpha'] +
                                          h, iter - h + self.priors[f]['beta'])
            posterior /= posterior.sum()
            
            # update the prior for the next round
            self.priors[f]['alpha'] = self.priors[f]['alpha'] + h
            self.priors[f]['beta'] = iter - h + self.priors[f]['beta']
        
            # calculate the bayes factor
            likelihood = sp.stats.binom.pmf(h, iter, self.theta)
            h0 = likelihood * self.prior
            h1 = likelihood * posterior
            bf = h1.sum()/h0.sum()
            
            if bf > self.bf_thresh:
                posterior_med = sp.stats.beta.median(self.priors[f]['alpha'], 
                                                      self.priors[f]['beta'])
                prior_med = sp.stats.beta.median(self.alpha, self.beta)
                prior_lower95 = sp.stats.beta.interval(.95, self.alpha, 
                                                       self.beta)[0]
                if posterior_med > prior_med:
                    dec_reg[bayes_features[i]] = 1
                elif posterior_med < prior_lower95:
                    dec_reg[bayes_features[i]] = -1
        return dec_reg
